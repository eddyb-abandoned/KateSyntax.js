KateSyntax.langs.e.syntax = {
    ...
    e_normal: function e_normal(m) {
        this.push();
        while(this.pos < this.len) {
            if(this.str[0] == '{' && this.hl('{', 'dsNormal;color:#404000;font-style:normal;font-weight:normal')) continue;
            if(this.str[0] == '}' && this.hl('}', 'dsNormal;color:#404000;font-style:normal;font-weight:normal')) continue;
            ...
            if((m = /^['[&><=:+\\\-*\\|\].,;]/.exec(this.str)) && this.hl(m[0], 'dsNormal;color:#404000;font-style:normal;font-weight:normal')) continue;
            ...
            if((m = /^(?:run|init|pre_generate|dut_error|pack|unpack|post_generate|pre_generate|set_config|hex|stop_run|append|size|delete|is_empty|deep_compare|deep_compare_physical|clear|pop0|setup|crc_32)\b/.exec(this.str)) && this.hl(m[0], 'dsFunction;font-weight:bold')) continue;
            if((m = /^(?:chars|define|extend|event|ECHO|DOECHO|import|initialize|non_terminal|struct|unit|script|testgroup|type)\b/.exec(this.str)) && this.hl(m[0], 'dsNormal;color:#8080FF;font-style:normal;font-weight:bold')) continue;
            if((m = /^(?:C|add|also|and|as|as_a|break|code|compute|computed|delayed|do|else|each|emit|empty|end|exit|finish|for|from|if|in|is|like|log|new|no|not|only|or|out|read|repeat|return|reverse|routine|step|then|to|traceable|untraceable|var|when|while|with|write|xor)\b/.exec(this.str)) && this.hl(m[0], 'dsNormal;color:#FF8080;font-style:normal;font-weight:bold')) continue;
