KateSyntax.langs.pgn.syntax = {
    ...
    pgn_comment_c: function pgn_comment_c(m) {
        ...
        while(this.pos < this.len) {
            ...
            if(this.str[0] == '(' && this.hl('(', 'dsComment')) {if(m = this.pgn_comment_p())return this.pop(), m-1;continue;}
            if(this.str[0] == '{' && this.hl('{', 'dsComment;font-weight:bold')) {if(m = this.pgn_comment_c())return this.pop(), m-1;continue;}
            if(this.str[0] == '}' && this.hl('}', 'dsComment;font-weight:bold')) return this.pop();
            this.hl(this.str[0], 'dsComment;font-weight:bold');
        }
        this.pop();
    },
    pgn_string: function pgn_string(m) {
        this.push();
        while(this.pos < this.len) {
            if(this.str[0] == '"' && this.hl('"', 'dsString;font-weight:normal')) return this.pop();
            this.hl(this.str[0], 'dsString;font-weight:normal');
        }
        this.pop();
    }
};
